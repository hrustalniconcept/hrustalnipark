...
PLANS_DIR = os.path.abspath("./assets/plans_web")

_GROUP_PREFIX = {
    "Дуплекс · дома 4 и 14": "д.4,14",
    "Дуплекс · дома 5, 12, 13": "д.5,12,13",
    "Дуплекс · дома 6 и 9": "д.6,9",
    "Дуплекс · дома 7 и 8": "д.7,8",
    "МКД · дом 10": "д.10",
    "МКД · дом 11": "д.11",
}
...
def plan_path(lot):
    return os.path.join(PLANS_DIR, f"{_GROUP_PREFIX[lot['group']]} №{lot['apt']}.png")
...
.planframe {
  ...
  flex-direction: column; gap: 4mm; position: relative; padding: 7mm;
}
.planframe.placeholder::before {
  content: ""; position: absolute; left: 6mm; top: 6mm; right: 6mm; bottom: 6mm;
  border: 1px solid #EFE9DF; border-radius: 1mm;
}
.plan-img { max-width: 100%; max-height: 100%; object-fit: contain; display: block; }
.plan-mark {
...
def build_page(lot):
    uri = plan_uri(lot)
    if uri:
        plan_inner = f'<img class="plan-img" src="{uri}">'
        plan_cls = "planframe"
    else:
        plan_inner = ('<div class="plan-mark">Планировка квартиры</div>'
                      '<div class="plan-hint">Вставьте изображение планировки</div>')
        plan_cls = "planframe placeholder"
